...
|---|---|---|---|---|
| global (3-class) | 0 | 0.540 | 0.425 – 0.655 | chance |
| global | 1 | 0.437 | 0.324 – 0.554 | chance |
| global | 2 | 0.495 | 0.390 – 0.606 | chance |
| local (2-class) | 0 | 0.520 | 0.408 – 0.628 | chance |
| local | 1 | 0.480 | 0.373 – 0.594 | chance |

The `normalized` variant — every image forced to 512×512 and re-encoded at JPEG
q90, to remove resolution and compression as a shortcut — reproduces this
almost exactly (global 0.529 / 0.458 / 0.485; local 0.520 / 0.480). Nothing was
...
**The delta attributable to the parsing bug is 0.472 → 0.445 AUROC — that is,
fixing the arithmetic makes the shipped number very slightly *worse*.** Both
...
is still correct — code that reads the wrong array positions must not ship — but
it must not be described as a fix to detection quality.
...
that when the noisy global score falls outside 0.05–0.95 the local model is
...
always runs. The gate is still bad design — it is guarded by a meaningless
...
0–255) × 2 resize policies (squash to 224, short-side resize then centre crop) ×
5 class indices = 50 measurements.

**The strongest separation found anywhere was AUROC 0.341** — 0.159 from chance,
and in the *inverted* direction. Across 50 measurements on a 100-image set, a
...
have destroyed a working head. It is unlikely — dynamic int8 on a ViT typically
costs a few points, not a model — and it cannot be checked, because no fp32
original exists in the repository. It is recorded here as the one alternative
explanation the evidence cannot close.

## Per-generator

All four generator families sit within noise, at n = 12–13 each. The intervals
...
2. **Fix `worker.ts`, and assert the tensor contract at load time** — class
   count, tensor names, normalisation constants read from the model rather than
   hardcoded — so this failure class breaks loudly instead of rendering "87% AI"
   from noise.
